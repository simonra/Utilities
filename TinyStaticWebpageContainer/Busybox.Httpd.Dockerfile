# Adapted from the blog post at https://lipanski.com/posts/smallest-docker-image-static-website
# With the associated repo at https://github.com/lipanski/docker-static-website
FROM alpine:3.13.2 AS compressor

RUN apk add gzip

# WORKDIR /StaticAssets

COPY WebsiteFiles/* /WebsiteFiles/

RUN gzip --recursive --best --keep /WebsiteFiles
...
# COPY . .
COPY --from=compressor /WebsiteFiles ./
